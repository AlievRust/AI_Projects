...
def generate_content(topic: str):
    ...
    try:
        ...
        title = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Придумай привлекательный и точный заголовок для поста в Телеграм на тему '{topic}' на русском языке, с учётом актуальных новостей:\n{recent_news}. Заголовок должен быть интересным и ясно передавать суть темы."
            }],
        ...
        meta_description = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"Напишите мета-описание для поста в Телеграм с заголовком: '{title}'. Оно должно быть на русском языке, полным, информативным и содержать основные ключевые слова."
            }],
        ...
        post_content = openai.ChatCompletion.create(
            ...
            messages=[{
                "role": "user", 
                "content": f"""Напишите подробный пост для Телеграм на тему '{topic}' на русском языке, используя последние новости:\n{recent_news}. 
                Пост должен отвечать требованиям:
                1. Быть информативныс, логичным и интересным
                2. Содержать не менее 1500 символов
                3. Иметь четкую структуру с подзаголовками
                4. Включать анализ текущих трендов
                5. Не иметь вступление и заключение
                ...
                9. Не содержать символы, которые требуют экранирования для автопостинга в Телеграм"""
            }],
